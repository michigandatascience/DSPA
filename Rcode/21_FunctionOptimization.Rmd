...
The symmetric positive definite matrix $D$ is:
...
The linear algebra and matrix computing [Chapter 4](http://www.socr.umich.edu/people/dinov/courses/DSPA_notes/04_LinearAlgebraMatrixComputing.html) and the regularized parameter estimation in [Chapter 17](http://www.socr.umich.edu/people/dinov/courses/DSPA_notes/17_RegularizedLinModel_KnockoffFilter.html) provide additional examples of least squares parameter estimation, regression and regularization.
...
One objective function we can design to denoise the observed signal, $x_{noisy}$, may include a *fidelity term* and a *regularization term*, see the regularized linear modeling, [Chapter 17](http://www.socr.umich.edu/people/dinov/courses/DSPA_notes/17_RegularizedLinModel_KnockoffFilter.html).
...
Let's look at another example, $\displaystyle\min_{x=(x_1,x_2)} f_1(x)=\min \{(\frac{1}{2}\times x_1^2-\frac{1}{4}\times x_2^2+3)\times \cos(2x_1+ 1- e^{x_2})\} $. 
...
Recall from [Chapter 4](http://www.socr.umich.edu/people/dinov/courses/DSPA_notes/04_LinearAlgebraMatrixComputing.html), that to estimate the linear regression coefficients on $p$ variables, we minimize the fidelity term:
...
Let's first solve the problem by hand using first-principles by [translating primary problem to a dual problem](http://www.socr.umich.edu/people/dinov/courses/DSPA_notes/21_FunctionOptimization.html#4_general_non-linear_optimization) using [Lagrange multipliers](http://www.socr.umich.edu/people/dinov/courses/DSPA_notes/21_FunctionOptimization.html#32_lagrange_multipliers). The *dual Profit function* will be $P^*(s_1, s_2, \lambda) = P(s_1, s_2)+\lambda\times constraint(s_1, s_2)$. To optimize it, we set $\nabla P^*=0$. This yields $\nabla P= -\lambda \nabla constraint$, i.e., 
...
The [Convex Optimization in R paper](http://dx.doi.org/10.18637/jss.v060.i05) provides lots of additional examples and [Wikipedia provides a number of interesting optimizaiton test functions](https://en.wikipedia.org/wiki/Test_functions_for_optimization).
...
* [Convex Optimization in R (DOI: 10.18637/jss.v060.i05)](http://dx.doi.org/10.18637/jss.v060.i05).
