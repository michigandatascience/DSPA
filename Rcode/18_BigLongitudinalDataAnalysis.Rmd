...
Let's search Google Trends for the volume for "data science" and "predictive analytics" searches in the US over the past ten years. Note that a time span can be specified via:
...
* "Y-m-d Y-m-d": Time span between two dates (ex.: "2008-06-22 2018-06-22")

```{r warning=F, error=F, message=F}
DSPA_trends_US <- gtrends(c("data science", "predictive analytics"), 
                          geo = c("US"), gprop = "web", 
                          time = "2008-06-22 2018-06-22")[[1]]
...
ggplot(data = DSPA_trends_US, aes(x = date, y = hits, group = keyword)) +
  ...
  ggtitle("Google Trends (Data Science, Predictive Analytics): 2008-2018")
```
...
ggplot(data = DSPA_trends_Global, aes(x = date, 
  ...
  ggtitle("Global <Data Science> Google Trends (US, CN, GB): 2008-2018")
```

What interesting patterns, trends and behavior is evident in the plot above?

Next, we can use the `prophet` package to automatically identify patterns provide forecasts of future searches. The `prophet()` function cal requires the input data-frame () to have columns "ds" and "y" representing the time-series dates and values, respectively.

```{r warning=F, error=F, message=F}
US_ts <- DSPA_trends_US[ , c("date","hits")]
...
plot(fit_US_ts_forecasts, include = 100, ylim=c(0,20))
```
