...
Later, in [Chapter 22](http://www.socr.umich.edu/people/dinov/courses/DSPA_notes/22_DeepLearning.html), we will provide more details and additional examples of deep neural network learning. For now, let's start by exploring the *magic* inside the machine learning black box.
...
$$y(x)=f\left (\sum_{i=1}^n w_ix_i\right ).$$
...
![](http://wiki.socr.umich.edu/images/3/36/DSPA_Figs_TwoLayer_Chapter6.png)
...
![](http://wiki.socr.umich.edu/images/0/07/DSPA_Figs_multioutput_Chapter6.png)
$$\text{Multi-output}$$
...
![](http://wiki.socr.umich.edu/images/f/f9/DSPA_Figs_delay_chapter6.png)
...
In this case study, we are going to use the [Google trends and stock market dataset](http://wiki.socr.umich.edu/index.php/SOCR_Data_GoogleTrends_2005_2011). A [doc file with the meta-data](https://umich.instructure.com/files/416273/download?download_frd=1) and the [CSV data](https://umich.instructure.com/files/416274/download?download_frd=1) are available on the [Case-Studies Canvas Site](https://umich.instructure.com/courses/38100/files/folder/Case_Studies). These daily data (between 2008 and 2009) can be used to examine the associations between Google search trends and the daily stock market index - Dow Jones Industrial average.
...
plot(google_model)
```
![](http://wiki.socr.umich.edu/images/e/e5/DSPA_Figs_plot_google_model_Ch6.png)

The above graph shows that we have only one hidden node. `Error` represents the aggregate sum of squared errors and `Steps` is how many iterations the model has go through. Note that these outputs could be different when you run exact same codes twice because the weights are stochastically estimated.
...
plot(google_model2)
```
![](http://wiki.socr.umich.edu/images/d/d1/DSPA_Figs_plot_google_model2_ch6.png)
...
We observe an even lower `Error` by use three hidden layer with nodes $4,3,3$, respectively. However, this complicates the interpretation of the resulting neural network.
...
Recall that in [Chapter 6](http://www.socr.umich.edu/people/dinov/courses/DSPA_notes/06_LazyLearning_kNN.html) we presented Lazy machine learning methods, which assign class labels using geometrical distances of different features. In multidimensional feature spaces, we can utilize spheres, centered according to the training dataset, to assign testing data labels. What kinds of shapes may be embedded in $nD$ space to help with the classification process?
...
[Chapter 21](http://www.socr.umich.edu/people/dinov/courses/DSPA_notes/21_FunctionOptimization.html#31_dual_problem_optimization) provides additional technical details about optimization duality. 
...
An alternative way to solve for the non-linear separable is called the *kernel trick*. That is to add some dimensions (or features) to make these non-linear separable data to be separable in a higher dimensional space. Below is one exampe where the 2D data doesn't appear as linearly separable in its native 2D.
...
p_2D <- plot_ly(mtcars, x = ~wt, y = ~hp/10, color = ~vs, colors = c('blue', 'red')) %>%
  add_markers() %>%
...
However, the data can be lifted in 3D where it's is more clearly linearly separable via a 2D plane.
...
w.col = ifelse(w=="V-Shaped Engine", "blue", "red")
...
z1 <- t(outer(x.seq, y.seq, z.seq))/10; range(z1)  # we need to check this 10 correction, to ensure the range of z is apporproate!!!

# Draw the 2D plane embedded in 3D, and then add points with "add_trace"
myPlotly <- plot_ly(x=~x.seq, y=~y.seq, z=~z1,
    colors = "gray", type="surface", opacity=0.5, showlegend = FALSE, showscale=FALSE) %>%
    add_trace(data=mtcars, x=x, y=y, z=mtcars$qsec, mode="markers", type="scatter3d", 
              marker = list(color=w.col, opacity=0.9, symbol=105), showscale = FALSE) %>%
  layout(showlegend = FALSE, showscale=FALSE, scene = list(
...
In this example, we use an optical document image (data) that has already been pre-partitioned into rectangular grid cells containing 1 character of the [26 English letters, A through Z](http://wiki.socr.umich.edu/index.php/SOCR_LetterFrequencyData).
...
![This figure show an example of the preprocessed gridded handwritten letters.](http://wiki.socr.umich.edu/images/2/2d/SOCR_Data_Dinov_EnglishLetterFrequency_Fig2.png)
...
Let's have another look at the *iris data* that we saw in [Chapter 2](http://www.socr.umich.edu/people/dinov/courses/DSPA_notes/02_ManagingData.html).
...
```{r,fig.height=6,fig.align='center',fig.width=8}
library(e1071)
...
We can see that only 1-2 cases of may be classified as Iris versicolor. The species of the majority of the flowers are all correctly identified.
...
Unfortunately, the model performance is actually worse than the previous one (you might get slightly different results). This is because this Iris dataset has a mostly linear feature space separation. In practice, we could try alternative kernel functions and see which one fits the dataset the best.
...
[Chapter 22](http://www.socr.umich.edu/people/dinov/courses/DSPA_notes/22_DeepLearning.html) provides more details about neural networks and deep learning.

# Practice

## Problem 1: Google Trends and the Stock Market

Use the [Google trend data](http://wiki.socr.umich.edu/index.php/SOCR_Data_GoogleTrends_2005_2011). Fit a neural network model with the same training data as [case study 1](http://www.socr.umich.edu/people/dinov/courses/DSPA_notes/10_ML_NN_SVM_Class.html#2_case_study_1:_google_trends_and_the_stock_market_-_regression). This time use `Investing` as target and `Unemployment, Rental, RealEstate, Mortgage, Jobs, DJI_Index, StdDJI` as predictors. Use 3 hidden nodes. 
