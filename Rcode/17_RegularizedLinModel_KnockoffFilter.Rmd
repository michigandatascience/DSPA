...
Prior to fitting regularized linear modeling and estimating the effects, covariates may be standardized. Scaling the features ensures the measuring units of the features do not bias the distance measures or norm estimates. Standardization can be accomplished by using the classic ["z-score" formula](http://wiki.socr.umich.edu/index.php/AP_Statistics_Curriculum_2007_Normal_Prob#General_Normal_Distribution]). This puts each predictor on the same scale (unitless quantities) - the mean is 0 and the variance is 1. We use $\hat{\beta_0} = \bar{y}$, for the mean intercept parameter, and estimate the coefficients of the remaining predictors. To facilitate interpretation of the results, after the model is estimated, in the context of the specific case-study, we can transform the results back to the original scale/units.
...
$$\arg\min_{{\beta}} \sum_{i=1}^n \left (y_i - \sum_{j=1}^{p} x_{ij} \beta_j \right )^2 = \arg\min_{{\beta}} \sum_{i=1}^{n} (y_i - x_{i}^T \beta)^2.$$
...
```{r setup, eval=TRUE, warning=F, message=F}
#### Initial Stuff ####
# clean up
rm(list=ls())
...
```{r LASSOpath, eval=TRUE, fig.align='center', fig.height=5.8, fig.width=8.5}
### Plot Solution Path ###
# LASSO
...
```{r ridgepath, eval=TRUE, fig.align='center', fig.height=5.8, fig.width=8.5}
### Plot Solution Path ###
# Ridge
...
```{r warning=FALSE, fig.height=7,fig.width=7,fig.cap="SSE Contour and Penalty Region (Ridge)."}
plot(p1)
```

```{r warning=FALSE, fig.height=7,fig.width=7,fig.cap="SSE Contour and Penalty Region (LASSO)."}
plot(p6)
```
...
```{r warning=FALSE, fig.height=14,fig.width=13,fig.cap="SSE Contour and Penalty Region for 6 values of Alpha."}
library("gridExtra")
...
```{r LASSOcv, eval=TRUE, fig.align='center', fig.height=5.8, fig.width=8.5}
...
```{r ridgecv, eval=TRUE, fig.align='center', fig.show='show'}
#### 10-fold cross validation ####
# Ridge Regression
set.seed(seed)  # set seed 
...
```{r regplot, eval=TRUE, fig.align='center', fig.height=5.3, fig.width=8.5}
...
```{r msetab, eval=TRUE, results='asis'}
# Test Set MSE Table 
# create table as data frame
