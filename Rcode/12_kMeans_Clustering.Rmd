...
subtitle: "<h2><u>k-Means Clustering</u></h2>"
...
This algorithm is similar to *k-nearest neighbors (KNN)* presented in [Chapter 6](http://www.socr.umich.edu/people/dinov/courses/DSPA_notes/06_LazyLearning_kNN.html). In clustering, we don't have a priori pre-determined labels, and the algorithm is trying to deduce intrinsic groupings in the data.
...
```{r echo=F, fig.width=7, fig.height=6, message=F, warning=F}
require(graphics)
...
* a [feature selection for clustering article](https://s3.amazonaws.com/academia.edu.documents/30779590/FS4clustering_chapter.pdf?AWSAccessKeyId=AKIAIWOWYYGZ2Y53UL3A&Expires=1521397760&Signature=aPPC5qI%2B5HY738mM2WyW0Ey0uw8%3D&response-content-disposition=inline%3B%20filename%3DFeature_Selection_for_Clustering_A_Revie.pdf) provides a review of possible strategies.
...
Use [Boys Town Study of Youth Development data](https://umich.instructure.com/courses/38100/files/folder/Case_Studies),  second case study, CaseStudy02_Boystown_Data.csv, we used in [Chapter 6](http://www.socr.umich.edu/people/dinov/courses/DSPA_notes/06_LazyLearning_kNN.html), to find clusters using variables about GPA, alcohol abuse, attitudes on drinking, social status, parent closeness and delinquency for clustering(all variables other than gender and ID).
...
myColors <- c("darkblue", "red", "green", "brown", "pink", "purple", "lightblue", "orange", "grey")
...
library("EBImage")
img <- img[ , , 1] # take the first RGB-color channel
# width and height of the original image
dim(img)[1:2]
olddim <- c(dim(img)[1], dim(img)[2])
newdim <- c(64, 64)  # new smaller image dimensions
img1 <- resize(img, w = newdim[1], h = newdim[2])
image(img, main="Original (high) resolution")
image(img1, main="Downsample (low) resolution)")
...
# L: normalized graph Laplacian, which can be caomputed in different ways:
...
# Compute the eigen-spectra for the nrmalized and unnormalized Laplacians 
...
image(segmatL, col=grey((0:15)/15), main="Normalized Laplacian")
image(segmatU, col=grey((0:15)/15), main="Unnormalized Laplacian")
...
image(seq(0, 1, length.out=olddim[1]), seq(0, 1, length.out=olddim[2]),
      img, col = grey((0:15)/15), xlim=c(-0.1, 1.1), 
      ylim=c(-0.1, 1.1), xlab="",ylab="", main="Original with Overlay of Boundary of the Unnormalized Laplacian Lable")
...
Let's try spectral clustering on the [Knee Pain Dataset](http://wiki.socr.umich.edu/index.php/SOCR_Data_KneePainData_041409) using the `kernlab::specc()` method.
