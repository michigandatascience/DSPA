...
Again, we see that the distances between pairs of points computed from original data and the simulated data coincide.
...
Let's use a subset (N=33) of [Parkinson's Progression Markers Initiative (PPMI) data](http://wiki.socr.umich.edu/index.php/SMHS_PCA_ICA_FA#Parkinson.27s_disease_case_study) to demonstrate the relationship between $S$ and PC loadings. First, we need to import the dataset into R and delete the patient ID column.
...
Suppose we want to fit a linear model `Top_of_SN_Voxel_Intensity_Ratio ~ Side_of_SN_Voxel_Intensity_Ratio + Part_IA`. We can plot the 3D scatterplot and the (univariate) linear model. 
...
However, we can also plot in 3D a bivariate 2D plane model (e.g., `lm.fit', or `pca1`) for the 3D scatter (Top_of_SN_Voxel_Intensity_Ratio, Side_of_SN_Voxel_Intensity_Ratio, Part_IA). Below is an example using the linear model.
...
### Fit (compute) the 2D PCA space (dimensionaltiy reduction)
...
# define the values of z = z(x.seq, y.seq), as a Matrix of dimension c(dim(x.seq), dim(y.seq))
#z <- t(outer(x.seq, y.seq, z.seq))/10; range(z)  # we need to check this 10 correction, to ensure the range of z is apporproate!!!
...
myPlotly <- plot_ly(x=~x.seq, y=~y.seq, z=~z,
  ...
  layout(scene = list(
    aspectmode = "manual", aspectratio = list(x=1, y=1, z=1),
    xaxis = list(title = "Side_of_SN_Voxel_Intensity_Ratio", range = c(min(x),max(x))),
    yaxis = list(title = "Top_of_SN_Voxel_Intensity_Ratio", range = c(min(y),max(y))),
    zaxis = list(title = "Part_IA"))
    )
# print(myPlotly)
myPlotly
...
To visualize how correlated the pre-processed data is and how independent our $S$ is we can draw the following two plots.
...
The `t-distributed` reference in t-SNE refers to the heavy-tailed *Student-t distribution* ($t_{df=1}$) which [councides](http://wiki.socr.umich.edu/index.php/AP_Statistics_Curriculum_2007_StudentsT) with [Cauchy distribution](http://wiki.socr.umich.edu/index.php/AP_Statistics_Curriculum_2007_Cauchy), $f(z)=\frac{1}{1+z^2}$. It is used to model and measure similarities between closer points in the embedded low-dimensional space, as well as dissimilarities of objects that map far apart in the embedded space. 
...
Later, in [Chapter 10](http://www.socr.umich.edu/people/dinov/courses/DSPA_notes/10_ML_NN_SVM_Class.html) and [Chapter 22](http://www.socr.umich.edu/people/dinov/courses/DSPA_notes/22_DeepLearning.html), we will present the Optical Character Recognition (OCR) and analysis of hand-written notes (unstructured text). 

Below, we show a simple example of generating a 2D embedding of the [hand-written digits dataset](http://www.socr.umich.edu/people/dinov/2017/Spring/DSPA_HS650/data/DigitRecognizer_TrainingData.zip) using t-SNE.
...
# Plot the result 2D map embedding of the data with digits or solid discs
...
# plot all cases as solid discs with colors corresponding to each of the 10 numbers
...
The [hands-on interactive SOCR t-SNE Dimensionaltiy Reduction Activity](http://socr.umich.edu/HTML5/SOCR_TensorBoard_UKBB) provides an interactive demonstration of t-SNE utilizing TensorBoard and the UK Biobank data.
...
Finally, let's try the t-Distributed Stochastic Neighbor Embedding method on the [PD data](http://wiki.socr.umich.edu/index.php/SOCR_Data_PD_BiomedBigMetadata). 
...
The results of the PCA, ICA, FA, and t-SNE methods on the [PD data](http://wiki.socr.umich.edu/index.php/SOCR_Data_PD_BiomedBigMetadata) imply that the data is complex and intrinsically high-dimensional, which prevents explicit embeddings into a low-dimensional (e.g., 2D) space. More advanced methods to interrogate this dataset will be demonstrated later. The [SOCR publications site](http://www.socr.umich.edu/people/dinov/publications.html) provides additional examples of Parkinson's disease studies.
